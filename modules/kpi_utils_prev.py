...
def compute_global_kpis(df):
    ...
    Parameters:
    - df (pd.DataFrame): DataFrame containing at least the 'insured_sum' column.
...
def generate_3d_scatter(df, mode="global"):
    """
    Generate a 3D scatter plot of latitude, longitude, and insured capital.

    Parameters:
        df (pd.DataFrame): DataFrame containing 'lat', 'lon', 'insured_sum', and 'zone' columns.
        mode (str): "global" for a single plot of all zones, "per_zone" to return one plot per zone.

    Returns:
        plotly.graph_objs.Figure or dict[str, Figure]: One 3D plot or a dictionary of plots by zone.
    """
