...
def haversine_coords(latitudes: np.ndarray, longitudes: np.ndarray) -> np.ndarray:
    """
    Convert decimal-degree lat/lon arrays into a Nx2 array of (lat_rad, lon_rad) for
    use with BallTree(metric='haversine').

    This function prepares geographic coordinates by converting from degrees to radians.
    ...
    Returns:
    - np.ndarray: Array of shape (n, 2) where each row is (latitude_rad, longitude_rad).
    """
    return np.column_stack((np.deg2rad(latitudes), np.deg2rad(longitudes)))


def euclidean_coords(latitudes: np.ndarray, longitudes: np.ndarray) -> np.ndarray:
    """
    Project decimal-degree lat/lon into a local planar x/y coordinate system (meters)
    for use with BallTree(metric='euclidean').

    Uses an equirectangular approximation centered at the mean latitude to minimize distortion
    over small areas.
...
def invert_euclidean(point_xy: Tuple[float, float],
                     lat0: float, lon0: float) -> Tuple[float, float]:
    """
    Inverse of the centered Euclidean projection:
    from (x_m, y_m) → (lat_deg, lon_deg), using lat0/lon0 in radians.
    """
    x_m, y_m = point_xy
...
def build_balltree(coords: np.ndarray, metric: str) -> BallTree:
    """
...
def tree_radius(radius_m: float, metric: str) -> float:
    """
    Convert a radius in meters into the units expected by BallTree.query_radius.

    For 'haversine', the tree expects radius in radians (fraction of Earth's radius).
...
def create_grid_around_point(center_point, radius, step, metric="haversine"):
    """
    Create a fine grid around a center point.
    
    Parameters:
    - center_point: tuple or array (lat_rad, lon_rad) for haversine or (x, y) for euclidean
    - radius: radius in tree units (radians for haversine, meters for euclidean)
    ...
    if metric == "haversine":
        ...
        lon_grid = np.arange(lon0 - radius/np.cos(lat0),
                            lon0 + radius/np.cos(lat0) + dlon, dlon)
        G_lat, G_lon = np.meshgrid(lat_grid, lon_grid)
        return np.column_stack((G_lat.ravel(), G_lon.ravel()))  # [lat, lon] in radians
    else:
        x0, y0 = center_point
        d = step
...
def create_incidence_matrix(neighbors_list, n_points, n_centers):
    """
    Create a sparse incidence matrix from a list of neighbors.
    
    Parameters:
    - neighbors_list: List of arrays where each array contains indices of neighbors
