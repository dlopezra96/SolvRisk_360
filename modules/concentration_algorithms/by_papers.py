...
# ────────────────────────────────────────────────────────────────────────────────
def exhaustive_concentration(df: pd.DataFrame, distance_type: str="haversine", 
                             radius: float=200) -> Tuple[np.array, float, Tuple[float, float]]:
    """
    Brute-force algorithm to find the group of policies within a given radius
    that maximizes the sum of insured values, using either Haversine or Euclidean distance.

    Parameters:
    - df (pd.DataFrame): DataFrame containing 'lat', 'lon', and 'insured_sum'.
    - distance_type (str): Type of distance to use ("haversine" or "euclidean").
    - radius (float): Maximum distance in meters to consider policies as a group.

    Returns:
    - best_group (np.ndarray): Indices of the higuest-sum cluster.
    - max_sum (float): Maximum total insured sum of that cluster.
    - best_center (Tuple[float, float]): Lat and lon where is located the center of highest cluster.
    """
    ...
    if distance_type == "haversine":
        ...
        raise ValueError("Unsupported distance_type. Use 'haversine' or 'euclidean' instead.")
...
def multi_start_continuous_search(
    ...
    - best_center (Tuple[float, float]): Lat and lon of the continuous optimal center.
    """
    ...
    def f(s: Tuple[float, float]) -> float:
        dx = xs - s[0]
        dy = ys - s[1]
        return insured[(dx*dx + dy*dy) <= radius2].sum()
    ...
    while (i < i_min) or (best_sum < Best):
        if i > 0:
            si = (rng.uniform(xs.min(), xs.max()), rng.uniform(ys.min(), ys.max()))
            delta = Delta0
        else:
            si = s0
            delta = Delta0
        ...
        while delta >= delta_min:
            ...
            neighbors = [
                (x_si + delta, y_si), (x_si - delta, y_si),
                (x_si, y_si + delta), (x_si, y_si - delta)
            ]
        ...
        if final_sum > best_sum:
            best_sum = final_sum
            best_center_xy = si
            best_group = np.where((xs - si[0])**2 + (ys - si[1])**2 <= radius2)[0]
...
# ────────────────────────────────────────────────────────────────────────────────
def grid_search_r(csv_path: str, radius: float=200.0, 
                  grid_distance: float=25) -> Tuple[int, float, Tuple[float, float], float]:
    """
    Fast grid search via R’s spatialrisk::highest_concentration().

    Parameters:
    - csv_path (str): Path to the CSV file (must contain columns lat, lon, insured_sum).
