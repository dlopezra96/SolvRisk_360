...
# ────────────────────────────────────────────────────────────────────────────────
def fast_exhaustive(df: pd.DataFrame, radius_m: float = 200.0,
                    metric: str = "haversine") -> Tuple[np.ndarray, float, Tuple[float, float]]:
    """
    ...
    4) Identify the center with maximum sum, return its indices, sum, and center coords.

    Parameters:
    - df (pd.DataFrame): DataFrame containing 'lat', 'lon', and 'insured_sum' columns.
    ...
    - best_center (Tuple[float, float]): (lat, lon) of the center achieving best_sum.
    """
...
# ────────────────────────────────────────────────────────────────────────────────
def fast_with_grid(df: pd.DataFrame, radius: float = 200.0, grid_distance: float = 25.0,
                   top_k: int = 10, metric: str = "haversine") -> Tuple[np.ndarray, float, Tuple[float, float]]:
    """
    Hybrid fast exhaustive + grid-refinement search for highest insured-sum cluster.
    ...
    4) Compare all continuous sums and pick the single best center across all anchors.
    ...
    - best_center (Tuple[float, float]): (lat, lon) of the continuous optimal center.
    """
    # Extract raw array
    lat = df["lat"].values
    lon = df["lon"].values
    ...
    M = coo_matrix((np.ones(len(rows), dtype=int), (rows, cols)), shape=(n, n))

    # Discrete sums at each original point
    disc_sums = M.dot(vals)
    ...
    for anchor_idx in top_idxs:
        ...
        G = create_incidence_matrix(grid_nbrs, n, len(grid))

        # Compute continuous sums
        grid_sums = G.dot(vals)
        ...
        if local_best_sum > best_cont_sum:
            ...
            if metric == "haversine":
                ...
                best_center = invert_euclidean(
                    tuple(grid[local_best_idx]), lat0, lon0
                )
...
# ────────────────────────────────────────────────────────────────────────────────
def fast_with_multilocal(df: pd.DataFrame, radius: float = 200.0, top_k: int = 10,
    ...
    for i, nbrs in enumerate(nbrs_list):
        rows.extend([i] * len(nbrs))
        cols.extend(nbrs.tolist())
    M = coo_matrix((np.ones(len(rows), int), (rows, cols)), shape=(n, n))
    disc_sums = M.dot(vals)
    ...
    if metric == "haversine":
        ...
        def f(pt):
            ...
            dlat = lat_rad - lat_c
            dlon = lon_rad - lon_c
            a = np.sin(dlat/2)**2 + np.cos(lat_rad)*np.cos(lat_c)*np.sin(dlon/2)**2
            d = 2 * np.arcsin(np.sqrt(a))
            return vals[d <= thr].sum()
        ...
        def f(pt):
            dx = xs - pt[0]
            dy = ys - pt[1]
            return vals[(dx*dx + dy*dy) <= thr].sum()
        delta0_cont    = delta0
    ...
    if metric == "haversine":
        ...
        best_center = invert_euclidean(
            tuple(coords_cont[best_disc_idx]), lat0, lon0
        )
