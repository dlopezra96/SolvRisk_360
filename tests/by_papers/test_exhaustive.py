...
from modules.concentration_algorithms.by_papers import exhaustive_concentration
...
def run_benchmark():
    ...
    print(f"🚀 Starting exhaustive_concentration benchmark...\n")
    ...
    for filename in datasets:
        ...
        for radius in RADIUS_LIST:
            for distance_type in DISTANCES:
                ...
                # --- Serial only ---
                try:
                    print(f"🔎 Serial → {distance_type.upper()} | Radius {radius}m")
                    start_time = time.perf_counter()
                    group, total_sum, best_center = exhaustive_concentration(df, distance_type=distance_type, radius=radius)
